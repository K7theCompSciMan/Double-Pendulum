...
# Parameters
buffer_size = 150  # Number of points to keep in the trail
...
bottom_lower = np.array([255, 255, 255])  # HSV lower bounds for green
bottom_upper = np.array([255, 255, 255])  # HSV upper bounds for green
fade_factor = 0.98  # How quickly points fade (smaller = faster fade)
...
def update_plot_midpoint():
    ...
    for i in midpoint_intensities:
        colors.append((1, 0, 0, i))  # RGBA red with varying alpha
...
def update_plot_bottom():
    x_points = [pt[0] for pt in bottom_pts]
    y_points = [pt[1] for pt in bottom_pts]
    colors = []
